...
exports.seedTier1ModulesOnSchedule = exports.getGate3Metrics = exports.checkTrainingDeadline = exports.recordTrainingProgress = exports.generateOriginalContent = exports.sendMonthlyReports = exports.checkModuleDeadlinesAndNotify = exports.onReminderCreated = exports.submitQuizAttempt = void 0;
...
async function submitQuizAttemptWithRetry(request, retryCount = 0) {
    try {
        return await submitQuizAttemptImpl(request);
    }
    catch (error) {
        if (retryCount < MAX_RETRIES && (error.code === "deadline-exceeded" || error.code === "aborted")) {
            v2_1.logger.warn(`submitQuizAttempt リトライ ${retryCount + 1}/${MAX_RETRIES}: ${error.message}`);
...
async function submitQuizAttemptImpl(request) {
    const auth = request.auth;
    if (!auth) {
        throw new https_1.HttpsError("unauthenticated", "サインインが必要です");
    }
    const { companyId, employeeId, moduleId, selectedAnswers } = request.data;
    if (!companyId || !employeeId || !moduleId || !Array.isArray(selectedAnswers)) {
        throw new https_1.HttpsError("invalid-argument", "companyId/employeeId/moduleId/selectedAnswersが必要です");
    }
    // Employeeドキュメント名は必ずauth.uidと一致させる方式(EmployeeService参照)なので、
    // 他人になりすましてクイズ結果を書き込めないようにここで検証する。
    if (auth.uid !== employeeId) {
        throw new https_1.HttpsError("permission-denied", "本人以外のクイズ結果は送信できません");
    }
    ...
    if (!companySnap.exists) {
        throw new https_1.HttpsError("not-found", "会社情報が見つかりません");
    }
    ...
    if (!employeeSnap.exists) {
        throw new https_1.HttpsError("permission-denied", "この会社に所属する社員が見つかりません");
    }
    ...
    if (globalModuleSnap.exists) {
        const module = globalModuleSnap.data();
        ...
        if (baseQuestionsSnap.empty && extensionQuestionsSnap.empty) {
            throw new https_1.HttpsError("failed-precondition", "この研修にはまだクイズ問題が登録されていません");
        }
        correctIndexes = [
            ...baseQuestionsSnap.docs.map((d) => d.data().correctIndex),
            ...extensionQuestionsSnap.docs.map((d) => d.data().correctIndex),
        ];
    }
    else {
        const customModuleSnap = await db.doc(`companies/${companyId}/customModules/${moduleId}`).get();
        if (!customModuleSnap.exists) {
            throw new https_1.HttpsError("not-found", "研修モジュールが見つかりません");
        }
        const customModule = customModuleSnap.data();
        ...
        correctIndexes = questionsSnap.docs.map((d) => d.data().correctIndex);
    }
    const correctCount = correctIndexes.reduce((count, correct, i) => (selectedAnswers[i] === correct ? count + 1 : count), 0);
...
exports.submitQuizAttempt = (0, https_1.onCall)(submitQuizAttemptWithRetry);
// ─────────────────────────────────────────────
// onReminderCreated: 管理者の個別リマインド送信(ReminderService.sendReminder)を
// トリガーにプッシュ通知を送る。
// ─────────────────────────────────────────────
exports.onReminderCreated = (0, firestore_1.onDocumentCreated)("companies/{companyId}/reminders/{reminderId}", async (event) => {
    const snapshot = event.data;
    if (!snapshot)
        return;
    const { companyId } = event.params;
    const { employeeId } = snapshot.data();
    if (!employeeId)
        return;
    const employeeSnap = await db.doc(`companies/${companyId}/employees/${employeeId}`).get();
    const employee = employeeSnap.data();
    if (!employee?.fcmToken) {
        v2_1.logger.info(`fcmToken未登録のためリマインド通知をスキップ: ${employeeId}`);
        return;
    }
...
exports.checkModuleDeadlinesAndNotify = (0, scheduler_1.onSchedule)({ schedule: "every day 09:00", timeZone: "Asia/Tokyo" }, async () => {
    ...
    for (const companyDoc of companiesSnap.docs) {
        ...
        const completedByEmployeeModule = new Set(enrollmentsSnap.docs
        ...
        for (const [moduleId, dueDateTimestamp] of Object.entries(deadlines)) {
            ...
            const daysRemaining = Math.round((jstDateOnlyMs(dueDate) - jstDateOnlyMs(new Date())) / DAY_MS);
            // リマインド期間より先、または大幅に過ぎた古い期限は対象外(通知の送りすぎを防ぐ)
            if (daysRemaining > REMINDER_WINDOW_DAYS || daysRemaining < -30)
                continue;
            ...
            for (const employeeDoc of employeesSnap.docs) {
                ...
                if (!employee.fcmToken)
                    continue;
                if (completedByEmployeeModule.has(`${employeeDoc.id}_${moduleId}`))
                    continue;
                try {
                    await messaging.send({
                        token: employee.fcmToken,
                    ...
                    v2_1.logger.warn(`通知送信に失敗しました employeeId=${employeeDoc.id}`, error);
                }
            }
        }
    }
...
// ─────────────────────────────────────────────
exports.sendMonthlyReports = (0, scheduler_1.onSchedule)({
    ...
    const periodStart = new Date(Date.UTC(jstNow.getUTCFullYear(), jstNow.getUTCMonth() - 1, 1) - JST_OFFSET_MS);
    const periodEnd = new Date(Date.UTC(jstNow.getUTCFullYear(), jstNow.getUTCMonth(), 1) - JST_OFFSET_MS);
    ...
    for (const companyDoc of companiesSnap.docs) {
        ...
        try {
            ...
            v2_1.logger.info(`月次レポートメール送信成功 companyId=${companyId} to=${contactEmail}`);
        }
        catch (error) {
            v2_1.logger.error(`月次レポートメール送信に失敗しました companyId=${companyId}`, error);
            ...
            try {
                ...
                v2_1.logger.error(`失敗通知メール送信にも失敗しました companyId=${companyId}`, notifyError);
            }
        }
    }
});
...
// ─────────────────────────────────────────────
const GENERATION_MODES = ["extend", "create"];
const THEME_MAX_LENGTH = 200;
exports.generateOriginalContent = (0, https_1.onCall)({ secrets: [anthropicApiKey], timeoutSeconds: 120 }, async (request) => {
    const auth = request.auth;
    if (!auth) {
        throw new https_1.HttpsError("unauthenticated", "サインインが必要です");
    }
    const { companyId, mode, theme, categoryId, targetModuleId } = request.data;
    if (!companyId || !mode || !theme) {
        throw new https_1.HttpsError("invalid-argument", "companyId/mode/themeが必要です");
    }
    if (!GENERATION_MODES.includes(mode)) {
        throw new https_1.HttpsError("invalid-argument", "modeはextendまたはcreateである必要があります");
    }
    if (theme.trim().length === 0 || theme.length > THEME_MAX_LENGTH) {
        throw new https_1.HttpsError("invalid-argument", `テーマは1〜${THEME_MAX_LENGTH}文字で入力してください`);
    }
    if (mode === "create" && !categoryId) {
        throw new https_1.HttpsError("invalid-argument", "新規モジュール作成にはcategoryIdが必要です");
    }
    if (mode === "extend" && !targetModuleId) {
        throw new https_1.HttpsError("invalid-argument", "既存モジュール追加にはtargetModuleIdが必要です");
    }
    ...
    if (!employeeSnap.exists || employee?.role !== "admin") {
        throw new https_1.HttpsError("permission-denied", "この会社の管理者のみ実行できます");
    }
    ...
    const hasAccess = mode === "create" ? tier === "moduleCreation" : tier === "moduleExtension" || tier === "moduleCreation";
    if (!hasAccess) {
        throw new https_1.HttpsError("permission-denied", mode === "create"
    ...
    if (mode === "extend" && targetModuleId) {
        ...
        if (!moduleSnap.exists) {
            throw new https_1.HttpsError("not-found", "対象のモジュールが見つかりません");
        }
    ...
    const lessonSchema = zod_2.z.object({
        title: zod_2.z.string().describe("レッスンのタイトル(20文字程度)"),
        body: zod_2.z
            .string()
            .describe("レッスン本文。200〜400文字程度で、具体的な職場での事例を交えて解説する"),
    });
    const quizQuestionSchema = zod_2.z.object({
        question: zod_2.z.string().describe("クイズの問題文"),
        choices: zod_2.z.array(zod_2.z.string()).length(4).describe("4択の選択肢(正解を1つ含む)"),
        correctIndex: zod_2.z.number().int().min(0).max(3).describe("正解の選択肢のインデックス(0始まり)"),
        explanation: zod_2.z.string().describe("なぜその選択肢が正解か、他が不正解かの解説"),
    });
    const schema = mode === "create"
        ? zod_2.z.object({
            moduleTitle: zod_2.z.string().describe("モジュールのタイトル(20文字程度)"),
            moduleDescription: zod_2.z.string().describe("モジュールの説明文(50〜100文字程度)"),
            lessons: zod_2.z.array(lessonSchema).length(lessonCount),
            quizQuestions: zod_2.z.array(quizQuestionSchema).length(quizCount),
        })
        : zod_2.z.object({
            lessons: zod_2.z.array(lessonSchema).length(lessonCount),
            quizQuestions: zod_2.z.array(quizQuestionSchema).length(quizCount),
        });
    const anthropic = new sdk_1.default({ apiKey: anthropicApiKey.value() });
    const prompt = "あなたは中小企業向け教育アプリ「安心企業研修Safy」のコンテンツ作成者です。\n" +
    ...
    while (retries <= maxRetries) {
        try {
            const response = await anthropic.beta.messages.parse({
                ...
                output_format: (0, zod_1.betaZodOutputFormat)(schema),
            });
            if (!response.parsed_output) {
                throw new Error("parsed_output is null");
            }
            parsed = response.parsed_output;
            v2_1.logger.info(`AIコンテンツ生成成功 companyId=${companyId} mode=${mode}`);
            return parsed;
        }
        catch (error) {
            ...
            if ((error.status === 429 || error.code === "deadline-exceeded") && retries < maxRetries) {
                ...
                v2_1.logger.warn(`AIコンテンツ生成リトライ ${retries + 1}/${maxRetries}: ${error.message}, ${Math.round(backoffMs)}ms後に再試行`);
                await new Promise((resolve) => setTimeout(resolve, backoffMs));
                retries++;
                continue;
            }
            v2_1.logger.error(`AIコンテンツ生成に失敗しました companyId=${companyId} mode=${mode}`, error);
    ...
    throw new https_1.HttpsError("internal", "AIコンテンツ生成に失敗しました。もう一度お試しください");
});
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
exports.recordTrainingProgress = (0, https_1.onCall)(async (request) => {
    const auth = request.auth;
    if (!auth) {
        throw new https_1.HttpsError("unauthenticated", "サインインが必要です");
    }
    const { companyId, employeeId, moduleId, score, passed } = request.data;
    if (!companyId || !employeeId || !moduleId || score === undefined || passed === undefined) {
        throw new https_1.HttpsError("invalid-argument", "companyId/employeeId/moduleId/score/passedが必要です");
    }
    if (score < 0 || score > 100) {
        throw new https_1.HttpsError("invalid-argument", "スコアは 0-100 の範囲である必要があります");
    }
    // Tier 1 Training モジュールのみ処理
    if (!moduleId.startsWith("tier1-")) {
        throw new https_1.HttpsError("invalid-argument", "この関数は Tier 1 Training モジュール向けです");
    }
    if (auth.uid !== employeeId) {
        throw new https_1.HttpsError("permission-denied", "本人以外の進捗は記録できません");
    }
    try {
        ...
        v2_1.logger.info(`Tier 1 Training 進捗記録: companyId=${companyId}, employeeId=${employeeId}, moduleId=${moduleId}, score=${score}, passed=${passed}`);
        ...
        const completedModules = await Promise.all(tierOneModuleIds.map(async (mid) => {
        ...
        v2_1.logger.error(`Tier 1 Training 進捗記録に失敗: ${error.message}`, error);
        throw new https_1.HttpsError("internal", "進捗記録に失敗しました");
    }
});
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
async function issueTrainingCertificate(companyId, employeeId, completedModuleIds) {
    try {
        // 既に発行済みか確認（重複発行防止）
        ...
        if (!existingCerts.empty) {
            v2_1.logger.info(`Tier 1 Training 修了証は既に発行済み: ${employeeId}`);
            return;
        }
        const now = new Date();
        ...
        v2_1.logger.info(`Tier 1 Training 修了証を発行しました: employeeId=${employeeId}, certificateNumber=${certificateNumber}`);
        // プッシュ通知を送信（修了証発行の通知）
        const employeeSnap = await db.doc(`companies/${companyId}/employees/${employeeId}`).get();
        const employee = employeeSnap.data();
        if (employee?.fcmToken) {
            try {
                ...
                v2_1.logger.warn(`プッシュ通知送信に失敗しました: ${notificationError}`);
            }
        }
    }
    catch (error) {
        v2_1.logger.error(`Tier 1 Training 修了証発行に失敗: ${error.message}`, error);
        throw error;
    }
}
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
exports.checkTrainingDeadline = (0, scheduler_1.onSchedule)("every day 00:00", async (context) => {
    const trainingDeadline = new Date("2026-09-22T23:59:59+0900"); // Sep 22 23:59 JST
    const now = new Date();
    // 期限を過ぎるまで実行しない
    if (now.getTime() <= trainingDeadline.getTime()) {
        v2_1.logger.info("Tier 1 Training 期限に到達していません");
        return;
    }
    try {
        ...
        for (const companySnap of companiesSnap.docs) {
            ...
            for (const employeeSnap of employeesSnap.docs) {
                ...
                const moduleResults = await Promise.all(tierOneModuleIds.map(async (moduleId) => {
                    const latestAttempt = await db
                        .collection(`companies/${companyId}/trainingAttempts`)
                        .where("employeeId", "==", employeeId)
                ...
                if (incompleteModules.length > 0) {
                    ...
                    v2_1.logger.warn(`Tier 1 Training 期限超過: companyId=${companyId}, employeeId=${employeeId}, incompleteModules=${incompleteModules.join(",")} `);
                }
            }
        }
        v2_1.logger.info("Tier 1 Training 期限切れチェック完了");
    }
    catch (error) {
        v2_1.logger.error(`Tier 1 Training 期限切れチェックに失敗: ${error.message}`, error);
    }
});
// ─────────────────────────────────────────────
// getGate3Metrics: GATE 3 検証メトリクスを集計
// ─────────────────────────────────────────────
exports.getGate3Metrics = (0, https_1.onCall)(async (request) => {
    const tierOneModuleIds = [
        "tier1-platform-tech",
        "tier1-operations",
        "tier1-content-production",
    ...
    try {
        ...
        for (const companySnap of companiesSnap.docs) {
            ...
            for (const employeeSnap of employeesSnap.docs) {
                ...
                const moduleResults = await Promise.all(tierOneModuleIds.map(async (moduleId) => {
                    ...
                    if (passed)
                        allModuleCompletions[moduleId].passed++;
                    return passed;
                }));
                if (moduleResults.every((m) => m))
                    allModulesPassed++;
            }
        }
        ...
        v2_1.logger.error(`GATE 3 メトリクス集計に失敗: ${error.message}`, error);
        throw new https_1.HttpsError("internal", "メトリクス取得に失敗しました");
    }
});
// ─────────────────────────────────────────────
...
exports.seedTier1ModulesOnSchedule = (0, scheduler_1.onSchedule)("2026-09-16 00:00:00 Asia/Tokyo", async (context) => {
    try {
        v2_1.logger.info("Tier 1 Training モジュールの自動シード開始");
        for (const moduleData of TIER1_MODULES_DATA) {
            ...
            if (existing.exists) {
                v2_1.logger.info(`${moduleData.id} は既に登録されています`);
                continue;
            }
            ...
            v2_1.logger.info(`Tier 1 Training モジュール登録完了: ${moduleData.id}`);
        }
        v2_1.logger.info("✅ Tier 1 Training モジュールの自動シード完了");
    }
    catch (error) {
        v2_1.logger.error(`Tier 1 Training モジュール自動シードに失敗: ${error.message}`, error);
    }
});
